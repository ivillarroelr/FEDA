...
void performMatrixExperiment(MatrixExperiments experiment,
  ...
  case matrixproduct: {
    ...
    mUtils.writeMatrixToOutputFile(resultMatrix,
                                   "output_matrixproduct_rectangle_for_" +
  ...
  case matrixproductoptimized: {
    ...
    mUtils.writeMatrixToOutputFile(resultMatrix,
                                   "output_matrixproductoptimized_rectangle_for_" +
                                       std::to_string(set) + ".txt",
                                   "output_files");
    // escribe en el archivo de estadisticas el tiempo de ejecucion
  ...
  case strassen: {
    ...
    mUtils.writeMatrixToOutputFile(
        resultMatrix, "output_strassen_rectangle_for_" + std::to_string(set) + ".txt",
        "output_files");
    // escribe en el archivo de estadisticas el tiempo de ejecucion
    outputFile << "strassen"
               << "," << set << "," << m << "," << duration << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 1) {
     ...
     */
    mUtils.generateMatrixFilesForProductAlgorithms(rectangle,
  ...
  for (int j = 1; j <= totalFilesMatrix; j++) {
    std::string filenameA = "matrix_rectangle_input_a_" + std::to_string(j) + ".txt";
    std::string filenameB = "matrix_rectangle_input_b_" + std::to_string(j) + ".txt";
    ...
    std::cout << "\r"
              << "Rectangle Matrix Working: " + std::to_string(percentage) + "%"
              << std::flush;

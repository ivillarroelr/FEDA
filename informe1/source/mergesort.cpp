...
void populateArrayAndExecuteExperiments(std::vector<int> &array,
  ...
  for (int i = 1; i <= totalFiles; i++) {
    ...
    std::cout << "\r"
              << "Mergesort Working: " + std::to_string(percentage) + "%"
              << std::flush;
    std::string filename;
    switch (type) {
    case randomized: {
    ...
    for (const auto &inputFile : std::filesystem::directory_iterator(dir)) {
      if ((inputFile.path().filename() == filename) &&
        ...
        if (file.is_open()) {
          ...
          sort.mergeSort(array, 0, array.size() - 1);
          ...
          sUtils.writeVectorToOutputFile(
              array, "output_mergesort_for_" + filename, "output_files");
          sortingOutputFile << "mergesort"
                            << "," << filename.substr(0, filename.length())
                            << "," << array.size() << "," << duration
                            << std::endl;
        } else {
...
int main(int argc, char *argv[]) {
  ...
  populateArrayAndExecuteExperiments(
      arrayOfNumbersSemi, input_dir_path_sorting_semi, filecounterSortingSemi,
      totalFilesSortingSemi, semi, sortingOutputFile);
